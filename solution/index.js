...
async function tellJoke(req, res) {
  ...
  const resp = await fetch(`https://v2.jokeapi.dev/joke/${params.jokeType}?safe-mode`);
  ...
  if (jokeResp.type === "single") {
      ...
      fulfillmentMessages: [
        {
          card: {
            title: `${params.jokeType} joke`,
            subtitle: jokeResp.setup + " " + jokeResp.delivery,
            buttons: [
              {
                text: "View More Jokes",
                postback: "https://v2.jokeapi.dev/"
    ...
    res.status(200).send({
      fulfillmentText: "Hmmm I don't know what happened... Please try again."
    })
  }
